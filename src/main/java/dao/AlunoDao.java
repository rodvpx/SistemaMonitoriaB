...
public class AlunoDao {
    ...
    public static Monitoria buscarPorId(int idMonitoria) throws SQLException {
        ...
        String sql = "SELECT d.nome AS disciplina_nome, d.codigo AS disciplina_codigo, l.sala, l.capacidade, l.inscritos, " +
                "h.dia_semana, h.horas, COUNT(im.id_aluno) AS total_inscritos, " +
                "m.id AS monitoria_id, m.id_monitor, m.id_supervisor " +
                "FROM monitoria m " +
                "JOIN disciplina d ON m.disciplina = d.codigo " +
                "JOIN local l ON m.local = l.id " +
                "JOIN horario h ON m.horario = h.id " +
        ...
        try (Connection conn = getConexao(); PreparedStatement sta = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = sta.executeQuery()) {
                if (rs.next()) {
                    ...
                    monitoria = new Monitoria(disciplina, horario, local, idMonitor, idSupervisor, totalInscritos, idMonitoria);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
